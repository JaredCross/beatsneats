...
var yelp = require('yelp').createClient({
  consumer_key: process.env.BEATSEATS_YELP_CONSUMERKEY,
  consumer_secret: process.env.BEATSEATS_YELP_CONSUMERSECRET,
  token: process.env.BEATSEATS_YELP_TOKEN,
  token_secret: process.env.BEATSEATS_YELP_TOKENSECRET
});
...
module.exports = {
  cityIdPromFunc : function (city) {
    var cityIdPromise = new Promise(function (resolve, reject) {
      unirest.get('http://api.songkick.com/api/3.0/search/locations.json?query='+ city+ '&apikey='+ process.env.BEATSEATS_SONGKICK_API)
  ...
  musicEventPromFuncPage1 : function (cityId) {
    var musicEventPromPage1 = new Promise(function (resolve, reject) {
      unirest.get('http://api.songkick.com/api/3.0/metro_areas/'+ cityId + '/calendar.json?apikey=' + process.env.BEATSEATS_SONGKICK_API)
      .end(function (response) {
        resolve(response);
      });
    });
  ...
  weatherPromFunc : function (city) {
    var weatherPromise = new Promise(function (resolve, reject) {
      unirest.post('http://api.openweathermap.org/data/2.5/forecast?q='+city+',us&APPID=' + process.env.BEATSEATS_WEATHER_API)
      .send()
      .end(function (response) {
